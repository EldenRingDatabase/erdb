...
def parse_args(on_generate, on_replicate, on_find_values, on_calculate_ar, on_changelog, on_source, on_map, on_icons, on_fetch_calc_data):
    ...
    parses_gamedata = argparse.ArgumentParser(add_help=False)
    default_gamedata = GameVersionRange.from_version(cfg.VERSIONS[0]) if len(cfg.VERSIONS) > 0 else None
    parses_gamedata.add_argument("--gamedata", "-g", nargs="+", default=default_gamedata, action=_GamedataAction, help="Game version range to source the data from.")

    sources_data = argparse.ArgumentParser(add_help=False)
    sources_data.add_argument("--game-dir", "-g", type=Path, required=True, help="Path to Elden Ring's \"Game\" directory, where the binary is located.")
    sources_data.add_argument("--ignore-checksum", action=argparse.BooleanOptionalAction, help="Ignore MD5 verification of thirdparty tools.")
    sources_data.add_argument("--keep-cache", action=argparse.BooleanOptionalAction, help="Keep the unpacked files, if any.")

    subs = parser.add_subparsers(title="subcommands", required=True)

    gen = subs.add_parser("generate", aliases=["gen"], help="List of items to generate.", parents=[parses_gamedata, outputs_json])
    ...
    calc = subs.add_parser("calculate-ar", aliases=["ar"], help="Calculate attack power of an armament", parents=[parses_gamedata])
    ...
    source = subs.add_parser("source", help="Extract gamedata from an UXM-unpacked Elden Ring installation (Windows only)", parents=[sources_data])
    ...
    pmap = subs.add_parser("map", help="Extract world map image from an UXM-unpacked Elden Ring installation (Windows only)", parents=[sources_data])
    ...
    icons = subs.add_parser("icons", help="Extract item images from an UXM-unpacked Elden Ring installation (Windows only)", parents=[sources_data])
