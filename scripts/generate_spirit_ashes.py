import json
import er_params
from erdb_common import update_nested
...
def make_spirit_ash_object(row: ParamRow, goods: ParamDict, upgrade_mats: ParamDict) -> Dict[str, Dict]:
...
def main():
    ...
    for row in goods.values():
        if is_base_spirit_ash(row):
            update_nested(spirit_ashes, make_spirit_ash_object(row, goods, upgrade_mats))

    spirit_ashes_full["SpiritAshes"] = spirit_ashes
    with open("./spirit-ashes.json", mode="w") as f:
        json.dump(spirit_ashes_full, f, indent=4)
